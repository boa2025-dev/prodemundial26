...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const match = MATCHES.find(m => m.id === 'G1')!;
    const row = matchRow({ id: match.id, local: match.local, visitante: match.visitante, kickoff: match.kickoff, label: 'Fase de Grupos · Grupo G' });
    const html = emailShell(
      '🇳🇿 ¡Tim Payne entra a la cancha HOY!',
      'Se viene el debut de Nueva Zelanda en el Mundial 2026... sí, con Tim Payne en el campo 👀. Hoy juegan vs. Irán a las 22hs (hora Argentina). No te quedes sin tu pronóstico ⚽🔥',
      row
    );
    ...
    const payload = emails.map(email => ({
      from: FROM,
      to: email,
      subject: '🚨 ¡HOY juega Tim Payne! Nueva Zelanda debuta a las 22hs',
      html,
    }));
