...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const round = KNOCKOUT_ROUNDS.find(r => r.id === 'R16')!;
    ...
    const html = emailShell(
      `⚔️ ¡${round.name} ya está disponible!`,
    ...
    const payload = emails.map(email => ({
      from: FROM,
      to: email,
      subject: `⚔️ ¡${round.name} ya está disponible! Completá tu pronóstico`,
      html,
    }));
    ...
    await db.doc('notifications/state').set(
      { notifiedPhases: FieldValue.arrayUnion('R16') },
      { merge: true }
    );
